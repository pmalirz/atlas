...
function isTenantPath(path: string) {
    const expected = withTenantUiPath(path);
...
test.describe('Authentication Flows', () => {
    ...
    test.describe('Registration and Email Verification', () => {
        test('should register a new user and receive verification email', async ({ page }) => {
            ...
            await expect(page).toHaveURL(isTenantPath('/'));
        ...
        test('should verify email using link from email', async ({ page }) => {
            ...
            await page.goto(withTenantUiPath(verifyUrl!));
    ...
    test.describe('Password Reset Flow', () => {
        ...
        test.beforeEach(async ({ page }) => {
            ...
            // Logout
            await page.goto(withTenantUiPath('/login')); // This might trigger logout if session exists
        });
        ...
        test('should request password reset and receive email', async ({ page }) => {
            ...
            await expect(page).toHaveURL(isTenantPath('/forgot-password'));
        ...
        test('should reset password using link from email', async ({ page }) => {
            ...
            await page.goto(withTenantUiPath('/forgot-password'));
            await page.getByTestId('forgot-password-email-input').fill(testEmail);
            await page.getByTestId('forgot-password-submit-btn').click();
            await expect(page.getByText('Check Your Email')).toBeVisible({ timeout: 10000 });
            ...
            // 3. Visit reset URL
            await page.goto(withTenantUiPath(resetUrl!));
            ...
            await expect(page).toHaveURL(isTenantPath('/login'));
        });
